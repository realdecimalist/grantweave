...
export function createAward(
  ...
  const app = db.prepare('SELECT status FROM applications WHERE id = ?').get(input.applicationId) as
    | { status: ApplicationStatus }
    | undefined;
  ...
  try {
    ...
    rethrowUnique(err, 'duplicate', `application ${input.applicationId} already has an award`);
  }
}
...
export function addBudgetLine(
  ...
  return transact(db, () => {
    ...
    try {
      ...
      rethrowUnique(err, 'duplicate', `award ${input.awardId} already budgets category ${input.category}`);
    }
  });
}
